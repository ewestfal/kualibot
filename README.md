...
# Deploying
Kualibot is deployed to our dev box on 104.236.16.61. Make sure redis is running
on that box:

```bash
ssh root@104.236.16.61
redis-cli ping
```

If that works, redis is up and running. If not, try `sudo service redis-server restart`.
...
You can start kualibot locally by running:
...
Then you can interact with kualibot by typing `kualibot help`.

    kualibot> kualibot help

    kualibot> animate me <query> - The same thing as `image me`, except adds a few
